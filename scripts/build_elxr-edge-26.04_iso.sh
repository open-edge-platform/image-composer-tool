...
build_elxr_edge_26_04_iso_image() {
  echo "Building ELXR Edge 26.04 iso Image. (using earthly built binary)"
  ...
  set +e
  output=$( sudo -S ./build/image-composer-tool build image-templates/elxr13/elxr-edge-26.04-x86_64-minimal-iso.yml 2>&1)
  build_exit_code=$?
  set -e
  
  # Check for the success message in the output
  if [ $build_exit_code -eq 0 ] && echo "$output" | grep -q "image build completed successfully"; then
    echo "ELXR Edge 26.04 iso Image build passed."
    if [ "$RUN_QEMU_TESTS" = true ]; then
      echo "Running QEMU boot test for ELXR Edge 26.04 ISO image..."
      if run_qemu_boot_test_iso "elxr-edge-26.04-x86_64-minimal"; then
        echo "QEMU boot test PASSED for ELXR Edge 26.04 ISO image"
      else
        echo "QEMU boot test FAILED for ELXR Edge 26.04 ISO image"
        exit 1
      fi
    fi
  else
    echo "ELXR Edge 26.04 iso Image build failed."
    echo "Build output:"
    echo "$output"
