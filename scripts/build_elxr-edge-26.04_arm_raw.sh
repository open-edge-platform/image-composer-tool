...
build_elxr_edge_26_04_arm_raw_image() {
  echo "Building ELXR Edge 26.04 ARM raw Image. (using image-composer-tool binary)"
  ...
  # Check disk space before building (require at least 12GB for ELXR Edge 26.04 images)
  if ! check_disk_space 12; then
    echo "Insufficient disk space for ELXR Edge 26.04 ARM raw image build"
    exit 1
  fi
  
  # Temporarily disable exit on error for the build command to capture output
  set +e
  output=$( sudo -S ./image-composer-tool build image-templates/elxr13/elxr-edge-26.04-aarch64-minimal-raw.yml 2>&1)
  build_exit_code=$?
  set -e
  
  # Check for the success message in the output
  if [ $build_exit_code -eq 0 ] && echo "$output" | grep -q "image build completed successfully"; then
    echo "ELXR Edge 26.04 ARM raw Image build passed."
    if [ "$RUN_QEMU_TESTS" = true ]; then
      echo "Running QEMU boot test for ARM ELXR Edge 26.04 raw image..."
      if run_qemu_boot_test "elxr-edge-26.04-aarch64-minimal"; then
        echo "QEMU boot test PASSED for ELXR Edge 26.04 ARM raw image"
      else
        echo "QEMU boot test FAILED for ELXR Edge 26.04 ARM raw image"
        exit 1
      fi
    ...
    echo "ARM ELXR Edge 26.04 raw Image build failed."
    echo "Build output:"
    echo "$output"
