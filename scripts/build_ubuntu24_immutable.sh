...
build_ubuntu24_immutable_raw_image() {
  echo "Building Ubuntu 24 immutable raw Image. (using earthly built binary)"
  ...
  set +e
  output=$( sudo -S ./build/image-composer-tool build image-templates/ubuntu24/ubuntu24-x86_64-edge-raw.yml 2>&1)
  build_exit_code=$?
  set -e
