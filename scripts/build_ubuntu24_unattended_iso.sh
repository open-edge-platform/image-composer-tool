...
run_qemu_boot_test_iso() {
  ...
  sudo bash -c "
    ...
    if grep -q \"\$SUCCESS_STRING\" \"\$LOGFILE\"; then
      ...
      result=0 # setting return value 0 instead of 1 until fully debugged
    fi
...
build_ubuntu24_unattended_iso_image() {
  echo "Building Ubuntu 24 unattended ISO image (using earthly built binary)"
  ...
  if [ $build_exit_code -eq 0 ] && echo "$output" | grep -q "image build completed successfully"; then
    echo "Ubuntu 24 unattended ISO image build passed."
    if [ "$RUN_QEMU_TESTS" = true ]; then
      echo "Running QEMU boot test for Ubuntu 24 unattended ISO image..."
      if run_qemu_boot_test_iso "minimal-os-image-ubuntu-unattended-24.04"; then
        echo "QEMU boot test PASSED for Ubuntu 24 unattended ISO image"
      else
        echo "QEMU boot test FAILED for Ubuntu 24 unattended ISO image"
        exit 1
      fi
    fi
  else
    echo "Ubuntu 24 unattended ISO image build failed."
    echo "Build output:"
    echo "$output"
    exit 1
  fi
