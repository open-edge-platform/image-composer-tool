...
build_elxr_edge_26_04_raw_image() {
  echo "Building ELXR Edge 26.04 raw Image. (using image-composer-tool binary)"
  ...
  if ! check_disk_space 12; then
    echo "Insufficient disk space for ELXR Edge 26.04 raw image build"
    exit 1
  fi
  ...
  if [ $build_exit_code -eq 0 ] && echo "$output" | grep -q "image build completed successfully"; then
    echo "ELXR Edge 26.04 raw Image build passed."
    if [ "$RUN_QEMU_TESTS" = true ]; then
      echo "Running QEMU boot test for ELXR Edge 26.04 raw image..."
      if run_qemu_boot_test "elxr-edge-26.04-x86_64-minimal"; then
        echo "QEMU boot test PASSED for ELXR Edge 26.04 raw image"
      else
        echo "QEMU boot test FAILED for ELXR Edge 26.04 raw image"
        exit 1
      fi
    ...
    echo "ELXR Edge 26.04 raw Image build failed."
    echo "Build output:"
    echo "$output"
