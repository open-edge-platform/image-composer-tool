...
build_emt3_immutable_raw_image() {
  echo "Building EMT3 immutable raw Image. (using earthly built binary)"
  ...
  set +e
  output=$( sudo -S ./build/image-composer-tool build image-templates/emt3/emt3-x86_64-edge-raw.yml 2>&1)
  build_exit_code=$?
  set -e
