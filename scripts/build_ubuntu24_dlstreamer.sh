...
build_ubuntu24_dlstreamer_image() {
  ...
  if [ $build_exit_code -eq 0 ] && echo "$output" | grep -q "image build completed successfully"; then
    echo "Ubuntu24 DLStreamer Image build passed."
    if [ "$RUN_QEMU_TESTS" = true ]; then
      echo "Running QEMU boot test for Ubuntu24 DLStreamer image..."
      if run_qemu_boot_test "ubuntu24-dlstreamer-x86_64"; then
        echo "QEMU boot test PASSED for Ubuntu24 DLStreamer image"
      else
        echo "QEMU boot test FAILED for Ubuntu24 DLStreamer image"
        exit 1
      fi
      cleanup_image_files raw
    fi
  else
    echo "Ubuntu24 DLStreamer Image build failed."
    echo "Build output:"
    echo "$output"
    exit 1
  fi
