...
build_elxr_edge_26_04_immutable_raw_image() {
  echo "Building ELXR Edge 26.04 immutable raw Image. (using earthly built binary)"
  ...
  if ! check_disk_space 15; then
    echo "Insufficient disk space for ELXR Edge 26.04 immutable raw image build"
    exit 1
  fi
  
  # Temporarily disable exit on error for the build command to capture output
  set +e
  output=$( sudo -S ./build/image-composer-tool build image-templates/elxr13/elxr-edge-26.04-x86_64-edge-raw.yml 2>&1)
  build_exit_code=$?
  set -e
  
  # Check for the success message in the output
  if [ $build_exit_code -eq 0 ] && echo "$output" | grep -q "image build completed successfully"; then
    echo "ELXR Edge 26.04 immutable raw Image build passed."
    if [ "$RUN_QEMU_TESTS" = true ]; then
      echo "Running QEMU boot test for ELXR Edge 26.04 immutable raw image..."
      if run_qemu_boot_test "minimal-os-image-elxr"; then
        echo "QEMU boot test PASSED for ELXR Edge 26.04 immutable raw image"
      else
        echo "QEMU boot test FAILED for ELXR Edge 26.04 immutable raw image"
        exit 1
      fi
    ...
    echo "ELXR Edge 26.04 immutable raw Image build failed."
    echo "Build output:"
    echo "$output"
