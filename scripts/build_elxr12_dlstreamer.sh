...
build_elxr12_dlstreamer_image() {
  echo "Building ELXR12 DLStreamer Image. (using ict binary)"
  echo "Ensuring we're in the working directory before starting builds..."
  cd "$WORKING_DIR"
  echo "Current working directory: $(pwd)"
  
  set +e
  output=$(sudo -S ./ict build image-templates/elxr12-x86_64-dlstreamer.yml 2>&1)
  build_exit_code=$?
  set -e
  
  if [ $build_exit_code -eq 0 ] && echo "$output" | grep -q "image build completed successfully"; then
    echo "ELXR12 DLStreamer Image build passed."
    if [ "$RUN_QEMU_TESTS" = true ]; then
      echo "Running QEMU boot test for ELXR12 DLStreamer image..."
      if run_qemu_boot_test "elxr12-dlstreamer-x86_64"; then
        echo "QEMU boot test PASSED for ELXR12 DLStreamer image"
      else
        echo "QEMU boot test FAILED for ELXR12 DLStreamer image"
        exit 1
      fi
      cleanup_image_files raw
    fi
  else
    echo "ELXR12 DLStreamer Image build failed."
    echo "Build output:"
    echo "$output"
    exit 1
  fi
