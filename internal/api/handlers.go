...
func (s *Server) CancelBuild(w http.ResponseWriter, _ *http.Request, id httpapi.BuildId) {
	acc, err := s.svc.CancelBuild(id)
	if err != nil {
		writeServiceError(w, err)
		return
	}
	writeJSON(w, http.StatusAccepted, fromCancelAccepted(acc))
}
