...
func TestComposeUnavailableCombination(t *testing.T) {
	s := newTestService(t)
	// A planned-but-not-ready combination (empty template, grayed out in the UI)
	// must be rejected rather than composed.
	s.manifest.Combinations = append(s.manifest.Combinations, Combination{
		Vertical: "fed-aero", SKU: "drone-image-bkc", Platform: "ptl", OS: "ubuntu26-server", ImageType: "raw", Template: "",
	})
	_, err := s.Compose(Selection{
		Vertical: "fed-aero", SKU: "drone-image-bkc", Platform: "ptl", OS: "ubuntu26-server", ImageType: "raw",
	})
	se, ok := err.(*Error)
	if !ok {
		t.Fatalf("err = %v, want *Error", err)
	...
	// Assert on the code so an unrelated 400 (missing field) can't make this
	// test pass by accident: the rejection must be a NO_MATCH.
	if se.Code != "NO_MATCH" {
		t.Errorf("error code = %q, want NO_MATCH", se.Code)
	}
}
