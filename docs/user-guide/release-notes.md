...
- Templates composed with `extends` instead of duplication: Several templates now inherit a base template rather than restating it. `emt3-x86_64-emf-raw.yml` and `emt3-x86_64-dlstreamer.yml` extend `emt3-x86_64-edge-raw.yml`, `emt3-x86_64-emf-rt-raw.yml` extends `emt3-x86_64-emf-raw.yml`, and `debian13-x86_64-bb-raw.yml` extends `debian13-x86_64-minimal-raw.yml`. This removes a 41-package block that had been copied verbatim into four EMT3 templates. The Ubuntu robotics stack is expressed as a three-level chain: `ubuntu24-x86_64-robotics-core-raw.yml` extends `ubuntu24-x86_64-minimal-raw.yml` and provides ROS 2 Jazzy, OpenVINO and Gazebo as a buildable image that is also a reusable parent, and `ubuntu24-x86_64-robotics-slam-raw.yml` extends that to add collaborative SLAM, RealSense and Intel NPU support in 42 non-comment lines. The standalone `ubuntu24-x86_64-robotics-jazzy-raw.yml` is retained for comparison. Note that all seven package repositories are declared in the core layer: repositories merge by `codename` and three of them share the codename `noble`, so they collapse if two layers in a chain both declare repositories — a descendant that declares none inherits the full list intact. Each derived template was verified with `resolve --full` to produce the same functional fields as before, with two documented exceptions, both a consequence of inheritance rather than of any change in intent. Because package lists are a union with no removal syntax, a derived template also installs its parent's packages — the robotics example documents the six GRUB packages this costs it. And because the three EMT3 templates now inherit `emt3-x86_64-edge-raw.yml`, they also inherit its three sample repositories (`company-internal`, `dev-tools`, `intel-openvino`), so `emf-raw` and `emf-rt-raw` resolve to three repositories where they previously declared none and `dlstreamer` resolves to six rather than three. Those entries are inert — their URLs are the literal placeholder `<URL>`, which `rpmutils` skips before fetching, and EMT3 is RPM-based so apt-source generation never runs for it — so the built image is unchanged. Each of the three templates notes this in its header.

**Fixed**

- `ci`: static gate for every image template: none of the 64 checked-in templates was verified by CI. No workflow ran `validate` or `resolve`, and no Go test resolves a real template — `internal/config/validate/validate_test.go` reads a single azl3 file and checks it against the JSON schema only, so the `extends`/overlay merge was tested against synthetic YAML but never against the templates in the repo. A template whose `extends:` parent was moved or renamed would break with nothing to catch it, and 43 of the 64 templates are never built on a pull request either. The new `Template Gate` workflow runs `validate` and `resolve --full` over every path in `git ls-files 'image-templates/*.yml'`, so coverage extends to templates added later without touching CI. It completes in ~5 s, needs no root, Docker, Earthly or apt, and contacts no package mirror, so it cannot flake on an upstream outage. `resolve` is the load-bearing half: it folds the extends chain and the OS defaults, turning a moved parent (`extends path not found`) or a `..` escape (`extends path escapes child template's directory`) into a build failure. Exit status alone is not trusted — a run from the wrong directory cannot find `./config`, warns `Could not load default configuration`, and still exits 0, so the gate fails on that string rather than reporting a vacuous pass.

- `fix(templates)`: `ubuntu24-x86_64-fde-raw.yml` referenced a passphrase file that exists on no machine: `systemConfig.fde.passphraseFile` was `/home/user/test-fde.txt`, an absolute path from a developer's home directory, making this the only template that failed `validate` on a fresh clone (`failed to read systemConfig.fde.passphraseFile … no such file or directory`). It now points at `fde-passphrase.example.txt`, committed beside the template; a relative `passphraseFile` resolves against the template's own directory, so the example travels with it. The value is a placeholder, not a usable secret, and the tool redacts it in `resolve` output. Commenting the field out instead is not an option — the schema requires `passphraseFile` whenever `fde.enabled` is true.
