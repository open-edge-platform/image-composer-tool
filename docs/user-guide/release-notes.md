...
- Overlay auto-sized disk grow (`disk.size`): Overlay builds can now **size the disk from the packages being installed** instead of requiring an exact size up front. With `overlayPolicy.allowDiskResize: true`, the tool estimates the space the resolved packages need (summing each package's repo-reported installed size — Debian `Installed-Size`, RPM `<size installed=…>` — with a conservative overhead factor and a fixed margin), measures the baseline's **real** free space, and grows the image only by the shortfall, never past `disk.size` when it is set. Leaving `disk.size` unset (with resize enabled) auto-sizes with no cap. The final image may be smaller than `disk.size`; if the computed need exceeds the ceiling the tool caps at it and warns (the install may then hit "no space left on device"). When no package reports a size, auto-sizing falls back to `disk.size` (or performs no grow when it too is unset). Resize remains grow-only and never shrinks the image.
