...
- Full offline/cache mode for DEB and RPM repositories: DEB Packages.gz metadata is now cached by SHA-256 checksum (`packages.parsed.json`) under `cache_dir/` and reused on rebuilds with no network access. RPM caches now include stable `repomd.xml`, checksum-bound exact-href primary metadata payloads (`primary_<hash>.gz`/`.zst`), `primary.location.json`, and parsed `primary.parsed.json` entries under `cache_dir/rpm-metadata/`, so a fully populated RPM cache can resolve dependencies without repository HTTP requests. Cache manifests store hashed metadata identities rather than repository URLs. Debian repository GPG keys are cached in `cache_dir/gpg-keys/`. Repository file-existence check results and package-list URLs are cached in-process per run to eliminate redundant HEAD requests.
