...
| 2026-07-02 | ICT Team | Documented in-memory session state (preserved across tabs, lost on refresh; no localStorage/server sessions for MVP-1); required `platform` on every combination — no platform-agnostic templates |
| 2026-07-04 | ICT Team | Package search adopts a live index reusing ICT's existing `debutils`/`rpmutils` repo-metadata parsers (name+version, prefix filter) instead of a static list |
| 2026-07-05 | ICT Team | Addressed PR #727 review: `ComposeRequest`/`ComposeResponse` wording aligned to manifest-lookup model (no merge/synthesis); corrected endpoint count; used explicit platform in data-model example; prototype fixes (build-status element, case-insensitive search, artifact-copy handler, OS/dist consistency, partition `size` in YAML) |
