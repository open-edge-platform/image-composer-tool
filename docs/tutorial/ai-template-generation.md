...
| Requirement | Details |
|-------------|---------|
| **ict** binary | Built via `earthly +build` or `go build ./cmd/ict` |
...
./ict ai "create a minimal edge image for ubuntu with SSH"

# 3. Search for relevant templates without generating
./ict ai --search-only "cloud image with monitoring"
```
...
./ict ai --provider openai "create a minimal elxr image for IoT"
```
...
./ict ai "create an edge image" --output /tmp/my-image.yml
```
...
Use `--provider openai` when running `ict ai`:

```bash
./ict ai --provider openai "minimal Ubuntu server image for cloud VMs"
```
...
| Poor search results | Stale cache | `./ict ai --clear-cache` |
...
- [Usage Guide](usage-guide.md) - General ict usage
