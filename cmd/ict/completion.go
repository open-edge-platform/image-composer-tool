...
func executeInstallCompletion(cmd *cobra.Command, args []string) error {
	...
	case "bash":
		...
		targetPath = filepath.Join(completionDir, "ict.bash")
	case "zsh":
		...
		targetPath = filepath.Join(completionDir, "_ict")
	case "fish":
		...
		targetPath = filepath.Join(completionDir, "ict.fish")
	case "powershell":
		...
		targetPath = filepath.Join(profilePath, "ict-completion.ps1")
	}
