...
// resolveDirOrFatal resolves a config directory and fails the test on error, so
// assertions never compare against an empty string from an ignored error.
func resolveDirOrFatal(t *testing.T, resolve func() (string, error)) string {
	t.Helper()
	dir, err := resolve()
	if err != nil {
		t.Fatalf("resolving directory: %v", err)
	}
	return dir
}
...
func TestSetupNoCache(t *testing.T) {
	...
	noCacheContext, cleanup, err := setupNoCache()
	if err != nil {
		t.Fatalf("setupNoCache failed: %v", err)
	}

	// Unique dirs should have been created.
	if _, err := os.Stat(noCacheContext.uniqueCacheDir); err != nil {
		t.Errorf("unique cache dir should exist: %v", err)
	}
	if _, err := os.Stat(noCacheContext.uniqueWorkDir); err != nil {
	...
	if err != nil {
		t.Fatalf("resolving work dir: %v", err)
	}
	if noCacheContext.uniqueCacheDir == absCache {
		t.Error("unique cache dir should differ from configured cache dir")
	}
	if noCacheContext.uniqueWorkDir == absWork {
		t.Error("unique work dir should differ from configured work dir")
	}
	if got, want := filepath.Dir(noCacheContext.uniqueCacheDir), filepath.Dir(absCache); got != want {
		t.Errorf("unique cache dir parent = %q, want %q", got, want)
	}
	if got, want := filepath.Dir(noCacheContext.uniqueWorkDir), filepath.Dir(absWork); got != want {
		t.Errorf("unique work dir parent = %q, want %q", got, want)
	}

	// The global config must now point at the unique dirs.
	if got := resolveDirOrFatal(t, config.CacheDir); got != noCacheContext.uniqueCacheDir {
		t.Errorf("config cache dir = %q, want %q", got, noCacheContext.uniqueCacheDir)
	}
	if got := resolveDirOrFatal(t, config.WorkDir); got != noCacheContext.uniqueWorkDir {
		t.Errorf("config work dir = %q, want %q", got, noCacheContext.uniqueWorkDir)
	}

	// origWorkDir must capture the pre-override configured workspace.
	if noCacheContext.origWorkDir != absWork {
		t.Errorf("origWorkDir = %q, want %q", noCacheContext.origWorkDir, absWork)
	}

	// Cleanup should remove both unique dirs.
	cleanup()
	if _, err := os.Stat(noCacheContext.uniqueCacheDir); !os.IsNotExist(err) {
		t.Errorf("unique cache dir should be removed after cleanup, stat err: %v", err)
	}
	if _, err := os.Stat(noCacheContext.uniqueWorkDir); !os.IsNotExist(err) {
		t.Errorf("unique work dir should be removed after cleanup, stat err: %v", err)
	}

	// Cleanup must also restore the original cache/work directories in the global config
	// so later code does not consult the removed directories.
	if got := resolveDirOrFatal(t, config.CacheDir); got != absCache {
		t.Errorf("config cache dir after cleanup = %q, want %q", got, absCache)
	}
	if got := resolveDirOrFatal(t, config.WorkDir); got != absWork {
		t.Errorf("config work dir after cleanup = %q, want %q", got, absWork)
	}
}
...
func TestSetupNoCache_CleanupKeepsWorkspaceOnFlag(t *testing.T) {
	...
	// The config must be restored even when the workspace is preserved for recovery.
	absCache, err := filepath.Abs(filepath.Join(tmp, "cache"))
	if err != nil {
		t.Fatalf("resolving cache dir: %v", err)
	}
	absWork, err := filepath.Abs(filepath.Join(tmp, "workspace"))
	if err != nil {
		t.Fatalf("resolving work dir: %v", err)
	}
	if got := resolveDirOrFatal(t, config.CacheDir); got != absCache {
...
func TestPreserveNoCacheOutput(t *testing.T) {
	...
	t.Run("CopiesImageOutput", func(t *testing.T) {
		...
		noCacheContext := &noCacheContext{uniqueWorkDir: uniqueWork, origWorkDir: origWork}
		if err := preserveNoCacheOutput(noCacheContext, providerId, configName); err != nil {
	...
	t.Run("NoOutputDirIsNoOp", func(t *testing.T) {
		...
		dst := filepath.Join(noCacheContext.origWorkDir, providerId, "imagebuild", configName)
