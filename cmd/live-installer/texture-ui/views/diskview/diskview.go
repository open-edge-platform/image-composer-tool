...
import (
	...
	"github.com/open-edge-platform/image-composer-tool/cmd/live-installer/texture-ui/views/diskview/autopartitionwidget"
	"github.com/open-edge-platform/image-composer-tool/cmd/live-installer/texture-ui/views/diskview/manualpartitionwidget"
...
func (dv *DiskView) Initialize(backButtonText string, template *config.ImageTemplate, app *tview.Application, nextPage, previousPage, quit, refreshTitle func()) (err error) {
	err = dv.populateBlockDeviceOptions()
	if err != nil {
		return
	}

	bootType := template.SystemConfig.Bootloader.BootType
	log.Infof("Boot type detected: %s", bootType)
	dv.autoPartitionWidget = autopartitionwidget.New(dv.systemDevices, bootType)
	err = dv.autoPartitionWidget.Initialize(backButtonText, template, app, dv.switchMode, nextPage, previousPage, quit, refreshTitle)
	if err != nil {
		return
	}

	dv.manualPartitionWidget = manualpartitionwidget.New(dv.systemDevices, bootType)
	err = dv.manualPartitionWidget.Initialize(backButtonText, template, app, dv.switchMode, nextPage, previousPage, quit, refreshTitle)
	if err != nil {
		return
	}
...
func (dv *DiskView) switchMode() {
	if dv.autoPartitionMode {
		...
		if err := dv.manualPartitionWidget.Reset(); err != nil {
			log.Errorf("Error resetting manual partition widget: %v", err)
			return
		}
		dv.manualPartitionWidget.SetSystemDeviceIndex(selectedBlockDevice)
		dv.pages.SwitchToPage(dv.manualPartitionWidget.Name())
	} else {
		if err := dv.autoPartitionWidget.Reset(); err != nil {
			log.Errorf("Error resetting auto partition widget: %v", err)
			return
		}
...
func (dv *DiskView) populateBlockDeviceOptions() (err error) {
	dv.systemDevices, err = imagedisc.SystemBlockDevices()
	return err
}
