...
const (
	...
	basePartitionUnit    = 1024 * 1024 // 1 MiB
...
func New(systemDevices []imagedisc.SystemBlockDevice, bootType string) *ManualPartitionWidget {
...
func (mp *ManualPartitionWidget) Initialize(backButtonText string, template *config.ImageTemplate, app *tview.Application, switchMode, nextPage, previousPage, quit, refreshTitle func()) (err error) {
...
func (mp *ManualPartitionWidget) validateAddPartitionForm() (err error) {
	...
	if sizeText[0] != maxPartitionSizeRune {
		...
		newPartitionSize, err = imagedisc.TranslateSizeStrToBytes(formattedSize)
		if err != nil {
			return
		}
...
func (mp *ManualPartitionWidget) populateTable() (err error) {
	...
	// Hardcode to GPT only
	bootPartitionMountPoint, _, _, err := imagedisc.BootPartitionConfig(mp.bootType, imagedisc.PartitionTableTypeGpt)
	if err != nil {
		return
	}
...
func (mp *ManualPartitionWidget) updateSpaceLabel() (err error) {
	...
	mp.spaceLeftText.SetText(fmt.Sprintf(uitext.DiskSpaceLeftFmt, imagedisc.TranslateBytesToSizeStr(bytesRemaining)))
...
func (mp *ManualPartitionWidget) addPartitionToTable(name, size, format, mountPoint string) (err error) {
	...
	sizeInBytes, err := imagedisc.TranslateSizeStrToBytes(size)
	if err != nil {
		return
	}
...
func (mp *ManualPartitionWidget) unmarshalPartitionTable() (err error) {
	...
	_, bootMountOptions, bootFlags, err := imagedisc.BootPartitionConfig(mp.bootType, partitionTableType)
	if err != nil {
		return
	}
	...
	partitions := make([]config.PartitionInfo, rows)
	...
	partitions[bootPartitionIndex].MountOptions = bootMountOptions
	...
	for i := range partitions {
		...
		partitions[i].Start = fmt.Sprintf("%dMiB", diskCursor/basePartitionUnit)
		partitions[i].End = fmt.Sprintf("%dMiB", nextCursor/basePartitionUnit)

		if partitions[i].MountPoint == rootMountPoint {
	...
	disk.Path = mp.systemDevices[mp.deviceIndex].DevicePath
...
func (mp *ManualPartitionWidget) mountPointInputValidation(textToCheck string, lastChar rune) bool {
	...
	if lastChar != '/' && !strings.ContainsRune(LegalCharactersAlphaNum, lastChar) {
