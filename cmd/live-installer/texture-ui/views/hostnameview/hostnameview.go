...
func (hv *HostNameView) Initialize(backButtonText string, template *config.ImageTemplate, app *tview.Application, nextPage, previousPage, quit, refreshTitle func()) (err error) {
	hv.defaultName = defaultHostName
	...
	hv.navBar = navigationbar.NewNavigationBar().
		...
		AddButton(uitext.ButtonNext, func() {
			...
			if err == nil {
				template.SystemConfig.HostName = enteredHostname
				nextPage()
			} else {
