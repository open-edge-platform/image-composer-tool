...
func (uv *UserView) Initialize(backButtonText string, template *config.ImageTemplate, app *tview.Application, nextPage, previousPage, quit, refreshTitle func()) (err error) {
...
func (uv *UserView) setupConfigUsers(template *config.ImageTemplate) (err error) {
	...
	newUserIndex := len(users) - 1
	uv.user = &users[newUserIndex]
